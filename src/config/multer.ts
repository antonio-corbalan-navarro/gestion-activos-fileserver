...
export const uploadDocsConfig = multer({
    storage: multer.diskStorage({
        destination: join(CURRENT_DIR, path.join('..', 'uploads', 'documents')),
    ...
    fileFilter: (req: Request, file: any, cb: Function) => {
        if (DOCS_MIMETYPES.includes(file.mimetype)) cb(null, true)
        else cb(new Error(`Only ${DOCS_MIMETYPES.join(' ')} mimetypes are valid`))
    },
    limits: {
        fieldSize: 20000000
    }
